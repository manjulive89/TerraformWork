...
resource "azurerm_virtual_machine" "ECB_VM" {
  ...
  network_interface_ids = ["${element(azurerm_network_interface.ECB_VM_Nic.*.id, count.index)}"]
  vm_size               = "${var.vm_size}"
...
resource "azurerm_network_interface_backend_address_pool_association" "ECB_BEPA" {
  count                = "${var.lb_attach == "true" ?  var.count : 0}"
  network_interface_id = "${azurerm_network_interface.ECB_VM_Nic.*.id[count.index]}"

  # network_interface_id = ["${element(azurerm_network_interface.ECB_VM_Nic.*.id, count.index)}"]
...
output "vm_nic_name" {
  value = "${azurerm_network_interface.ECB_VM_Nic.*.name}"
}
